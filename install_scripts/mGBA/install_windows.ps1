...
Write-Host "STATUS: Starting mGBA Emulator installation..."
...
$DownloadUrl = "https://github.com/mgba-emu/mgba/releases/download/0.10.5/mGBA-0.10.5-win64.7z"
$FileName = "mGBA-0.10.5-win64.7z"
$ArchiveFilePath = Join-Path -Path $InstallDirectory -ChildPath $FileName

Write-Host "STATUS: Downloading mGBA archive from $DownloadUrl..."
try {
...
# Locate the folder created by 7-zip (usually named something like 'mGBA-0.10.5-win64')
$ExtractedDir = Get-ChildItem -Path $EmulatorDir -Directory | Where-Object { $_.Name -like "mGBA-*" } | Select-Object -First 1
...
if ($ExtractedDir) {
    ...
    Write-Warning "WARNING: Could not automatically locate the extracted mGBA folder to rename."
}
