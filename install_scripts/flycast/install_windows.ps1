...
$DownloadUrl = "https://github.com/flyinghead/flycast/releases/download/v2.6/flycast-win64-2.6.zip"
$FileName = "flycast-win64-2.6.zip"
$ArchiveFilePath = Join-Path -Path $InstallDirectory -ChildPath $FileName

Write-Host "STATUS: Downloading flycast archive from $DownloadUrl..."
try {
...
Write-Host "STATUS: Generating local configuration file for portability..."
$ConfigPath = Join-Path -Path $EmulatorDir -ChildPath "emu.cfg"
if (!(Test-Path -Path $ConfigPath)) {
    New-Item -ItemType File -Path $ConfigPath -Force | Out-Null
    Write-Host "STATUS: Created emu.cfg."
}
