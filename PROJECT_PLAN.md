...
## Phase 1: Core Foundation & Modular Backend
...
## Phase 3: Main Application UI & Input
1. **Design Main Scene:** Create the main scene layout (System list, Game list, Details panel).
2. **Implement UI Components:** Build the UI elements for displaying the data.
3. **Handle Multiple Controllers:** Implement robust input handling to ensure that multiple game controllers can navigate the UI and are properly mapped.

## Phase 4: Feature Implementation (RomM)
1. **Data Display:** Fetch and populate the UI with data from the `RommAPI` backend.
2. **Search and Filtering:** Add a search bar to filter the game list.

## Phase 5: Configuration & Settings
1. **Application Settings Menu:** Create a UI for configuring global application settings.
2. **Local Paths Configuration:** Implement a configuration file system (`ConfigManager.cs`) to store and retrieve user preferences, specifically the base path for locally stored ROMs (defaulting to `user://roms/`).
