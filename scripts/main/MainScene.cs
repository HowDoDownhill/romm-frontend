...
public partial class MainScene : Control
{
	...
	private void SetupSettingsTree()
	{
		...
		TreeItem platformsItem = settingsSectionsTree.CreateItem(root);
		platformsItem.SetText(0, "Platform Settings");

		if (appInstance.dataBus.systems != null)
		{
			foreach (var system in appInstance.dataBus.systems)
			{
				TreeItem sysNode = settingsSectionsTree.CreateItem(platformsItem);
				sysNode.SetText(0, system.Name);
				sysNode.SetMetadata(0, system.Slug);

				GeneratePlatformSettingsForm(system);
			}
		}
