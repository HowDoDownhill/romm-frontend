...
- **DownloadManager**: Manages game ROM downloads by streaming through `System.Net.Http.HttpClient` (deliberately *not* Godot's `HttpRequest` — see DESIGN-NOTES). Supports multiple concurrent downloads with progress tracking (emits `DownloadProgressUpdated` each frame for active downloads), phase text (`DownloadStageChanged`) and completion notification (`DownloadCompleted`). Provides `DownloadFile()`, `IsDownloadingGame()`, `CancelDownload()`, `ReportDownloadStage()` and `CloseDeferredDownload()`. Transfers it does not own — emulator installs, save sync, the app updater — publish through the same signals via `BeginExternalTransfer()` / `ReportExternalTransferProgress()` / `ReportExternalTransferStage()` / `CompleteExternalTransfer()`, so everything the user waits on lands on the downloads page.
